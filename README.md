...
Verdent 社媒增长工作的独立云端 OS。它统一管理版本更新、新模型、编辑内容、Builder Story、服务通知、合作活动和事件活动，并将原始资料、内容、设计 Brief、任务素材、发布记录和复盘放在同一个任务中。
...
| 审核 | 自动扫描并完成最后自检 | 风险处理完毕，素材版本明确 |
| 发布 | 自动带入排期、选择账号并记录 URL | 状态、账号、格式和发布链接完整 |
| 复盘 | 记录表现并实时生成可视化摘要 | 将重复证据沉淀到模板或 Skill |

## 核心能力

- 自动识别 Brief，并发现多模型或多版本串线
- 七类任务、标准 / 紧急两套流程和可复用任务模板
- 管理 X、Discord、LinkedIn、Reddit、Instagram 和 TikTok 内容
- 统一维护官网和社媒账号目录
- 直接生成六平台英文初稿，或生成可见、可复制、可下载的 AI Prompt
- 管理海报、视觉和视频剪辑 Brief
- 拦截未确认上线表达、夸大词、隐私暗示、长破折号和平台格式问题
- 保存文案快照、发布链接、平台表现和复盘结论
- 列表、流程看板和日程视图，以及保留 30 天的任务回收站
- 北京时间默认、任务时区切换和导出时多时区标注
- 完整 JSON 备份与恢复
- 云端保存任务与账号数据，并自动迁移旧版浏览器数据
- 使用任务素材库保存图片、视频和设计文件，大文件采用分片上传
- 使用数据修订号拦截多设备覆盖冲突
...
网页中的“生成 AI Prompt”会显示并尝试复制一份包含事实状态、Brief、启用平台、账号和多时区排期的结构化任务；剪贴板不可用时仍可手动复制或下载。Codex 从项目相对路径读取 Skill，因此仓库移动或重新克隆后无需修改个人电脑路径。
