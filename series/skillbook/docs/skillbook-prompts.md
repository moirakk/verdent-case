...
- 复用 EP001 的时间轴比例：0-2.2s / 2.2-5.0s / 5.0-10.2s / 10.2-15.0s / 15.0-24.0s
- 总时长 24s，--T 设为 24000
...
- 一句话转化：<模糊输入> → <可用产出>
- hook 文案：<两行大字>
- raw input 引用：<一句话 + 3个tag + 若干碎片>
- 处理场景：<输入碎片 + skill core + 4个输出节点 + 状态文案序列>
...
2. 5段场景时间占比是否与 EP001 一致（0-2.2 / 2.2-5.0 / 5.0-10.2 / 10.2-15.0 / 15.0-24.0）
3. 色板顺序是否为 sage → cream → canvas → plum → sage
4. 是否只有一次主要"揭晓闪光"，且落在 artifact 场景开场
5. 文字是否在 390px 宽度下不溢出安全边距
6. next-tag 文案是否正确指向系列下一集（或末集留空）
7. 字体是否仅使用 Fraunces / Inter / IBM Plex Mono
8. 最终结果页是否在浅色内容区域内包含 Verdent 墨版 logo
```
