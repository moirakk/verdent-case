...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {newOpen && <div className="overlay" onClick={() => setNewOpen(false)}><section className="new-modal" onClick={(event) => event.stopPropagation()}><header><div><span className="eyebrow">NEW CONTENT TASK</span><h2>先把原始信息放进来</h2><p>可直接粘贴文字，也可上传文件和截图。链接会作为原始资料保留，但系统暂不读取链接内容；请把文档内容一并粘贴进来，Brief 识别才能生效。</p></div><button onClick={() => setNewOpen(false)}>×</button></header><label className="template-select">任务模板<select value={newTemplateId} onChange={(event) => setNewTemplateId(event.target.value)}><option value="">不使用模板</option>{templates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label><div className="kind-picker">{(["自动识别", ...kinds] as const).map((item) => <button className={newKind === item ? "active" : ""} onClick={() => { setNewKind(item); setNewTemplateId(""); }} key={item}>{item}</button>)}</div><textarea autoFocus value={newSource} onChange={(event) => setNewSource(event.target.value)} placeholder="粘贴 Changelog、模型上线通知、文档正文或内容想法……（链接仅保存，不会自动读取）" /><div className="intake-files"><button className="button" onClick={() => intakeRef.current?.click()}>＋ 上传文件或截图</button><input ref={intakeRef} hidden multiple type="file" accept="image/*,video/*,.txt,.md,.json,.csv,.pdf,.doc,.docx,.ppt,.pptx" onChange={(event) => void addIntakeFiles(event.target.files)} /><div>{newFiles.map((file, index) => <span key={`${file.name}-${index}`}>{file.name}<button aria-label={`移除 ${file.name}`} onClick={() => setNewFiles((items) => items.filter((_, itemIndex) => itemIndex !== index))}>×</button></span>)}</div><small>文本文件会自动读入；图片、视频和办公文件会作为云端原始素材保留。</small></div><footer><span>{newSource.length} 字 · {newFiles.length} 个文件</span><div><button className="button" onClick={() => setNewOpen(false)}>取消</button><button className="button primary" onClick={createTask}>建立任务并识别 →</button></div></footer></section></div>}
